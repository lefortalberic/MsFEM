...
for l=2:(Nbpt_H-2)   %La première et dernière ligne doivent rester nulle
    ...
    for j=1:(Nbpt_h-1)
        ...
        Somme_K_1 = Somme_K_1 + psi_g*psi_g_adjoint*A((l-2)*H+(j-0.5)*h,epsilon, Num_coeff)*((partie_gauche_fct_forme_gauche(j+1)-partie_gauche_fct_forme_gauche(j))/h)^2; %gradient de Khi_i au carré
        Somme_K_1 = Somme_K_1 + psi_d*psi_d_adjoint*A((l-1)*H+(j-0.5)*h,epsilon, Num_coeff)*((partie_droite_fct_forme_gauche(j+1)-partie_droite_fct_forme_gauche(j))/h)^2; %gradient de Khi_i au carré
    end
